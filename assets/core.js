...
(function(){
  ...
  paint();
  /* Only the nodes that were just added need swapping. Re-scanning every
     [style] element in the document on each mutation meant the terminal, the
     SQL builder and the data cleaner were triggering a full-document sweep on
     every keystroke. */
  function paintNodes(list){
    list.forEach(function(el){
      ...
      if (orig === null){
        ...
        RE.lastIndex = 0;
        if (!RE.test(st)) return;
        RE.lastIndex = 0;
        orig = st;
        el.setAttribute('data-lightstyle', orig);
      }
      el.setAttribute('style', dark() ? orig.replace(RE, function(m){ return SWAP[m.toLowerCase()] || m }) : orig);
    });
    if (window.__inkChips) window.__inkChips();
  }
  ...
  new MutationObserver(function(records){
    for (var r = 0; r < records.length; r++){
      ...
      for (var i = 0; i < added.length; i++){
        ...
        if (n.hasAttribute && n.hasAttribute('style')) pending.push(n);
        if (n.querySelectorAll){
          var inner = n.querySelectorAll('[style]');
